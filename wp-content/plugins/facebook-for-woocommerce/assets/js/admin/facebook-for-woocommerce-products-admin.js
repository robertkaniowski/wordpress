...
jQuery(document).ready(function($) {
  ...
  if ("product" === pagenow) {
    ...
    const facebookSettingsPanel = syncModeSelect.closest(
    ...
    $(".woocommerce_variations").on(
      ...
      function() {
        toggleFacebookSettings(
          $(this).val() !== "sync_disabled",
          $(this).closest(".wc-metabox-content")
        );
    ...
    $('form#post input[type="submit"]').on("click", function(e) {
      ...
      let $submitButton = $(this),
        ...
        productTag = $('textarea[name="tax_input[product_tag]"]').length
        ...
        varSyncEnabled =
          $(".js-variable-fb-sync-toggle").val() !== "sync_disabled";
      ...
      $(
        '#taxonomy-product_tag input[name="tax_input[product_tag][]"]:checked'
      ).each(function() {
      ...
      if (productID > 0) {
        $.post(
          facebook_for_woocommerce_products_admin.ajax_url,
          {
            ...
            security:
              facebook_for_woocommerce_products_admin.set_product_sync_prompt_nonce,
