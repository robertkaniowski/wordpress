jQuery(document).ready(function($) {
  var templates = {
    default: function(envelope) {
      ...
      html +=
        '<div class="jitm-banner__title">' +
        envelope.content.message +
        "</div>";
      if (envelope.content.description && envelope.content.description !== "") {
        html +=
          '<div class="jitm-banner__description">' +
          envelope.content.description;
      ...
      if (envelope.CTA.message) {
        ...
        html +=
          ...
          envelope.url +
          '" target="' +
          (envelope.CTA.newWindow === false || ajaxAction
            ? "_self"
            : "_blank") +
    ...
    $template.find("#jitm-banner__activate a").click(function() {
      ...
      $.ajax({
        ...
        beforeSend: function(xhr) {
          ...
          $("#jitm-banner__activate a").text(
            window.jitm_config.activating_module_text
          );
          $("#jitm-banner__activate a").attr("disabled", true);
        }
      }).done(function() {
        $("#jitm-banner__activate a").text(
          window.jitm_config.activated_module_text
        );
        $("#jitm-banner__activate a").attr("disabled", true);
  ...
  var reFetch = function() {
    ...
    $(".jetpack-jitm-message").each(function() {
      ...
      var full_jp_logo_exists = $(".jetpack-logo__masthead").length
        ? true
        : false;
