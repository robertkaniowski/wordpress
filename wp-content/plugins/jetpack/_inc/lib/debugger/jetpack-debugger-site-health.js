...
jQuery(document).ready(function($) {
  var JetpackSync = {
    ...
    checkProgress: function() {
      $.post(
        jetpackSiteHealth.ajaxUrl,
        { action: "jetpack_sync_progress_check" },
        function(response) {
          if ("done" === response) {
    ...
    setProgress: function() {
      if ("true" === JetpackSync.accordionIsOpen()) {
        ...
        $(".jetpack-sync-progress-bar").progressbar({
          value: JetpackSync.progressPercent
        });
        $(".jetpack-sync-progress-label").text(
          JetpackSync.progressPercent + "%"
        );
        ...
        JetpackSync.accordionButton()
          .find(".title")
          .text(
            jetpackSiteHealth.syncProgressHeading +
              " - " +
              JetpackSync.progressPercent +
              "%"
          );
