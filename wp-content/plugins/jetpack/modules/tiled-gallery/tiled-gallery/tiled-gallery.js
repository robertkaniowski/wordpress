(function($) {
  ...
  TiledGallery.prototype.getExtraDimension = function(el, attribute, mode) {
    if (mode === "horizontal") {
      var left =
        attribute === "border" ? "borderLeftWidth" : attribute + "Left";
      var right =
        attribute === "border" ? "borderRightWidth" : attribute + "Right";
      return (
        (parseInt(el.css(left), 10) || 0) + (parseInt(el.css(right), 10) || 0)
      );
    } else if (mode === "vertical") {
      var top = attribute === "border" ? "borderTopWidth" : attribute + "Top";
      var bottom =
        attribute === "border" ? "borderBottomWidth" : attribute + "Bottom";
      return (
        (parseInt(el.css(top), 10) || 0) + (parseInt(el.css(bottom), 10) || 0)
      );
    } else {
      return 0;
    }
  ...
  TiledGallery.prototype.resize = function() {
    ...
    this.gallery.find(this.resizeableElementsSelector).each(function() {
      ...
      var marginWidth = self.getExtraDimension(
        thisGalleryElement,
        "margin",
        "horizontal"
      );
      var marginHeight = self.getExtraDimension(
        thisGalleryElement,
        "margin",
        "vertical"
      );

      var paddingWidth = self.getExtraDimension(
        thisGalleryElement,
        "padding",
        "horizontal"
      );
      var paddingHeight = self.getExtraDimension(
        thisGalleryElement,
        "padding",
        "vertical"
      );

      var borderWidth = self.getExtraDimension(
        thisGalleryElement,
        "border",
        "horizontal"
      );
      var borderHeight = self.getExtraDimension(
        thisGalleryElement,
        "border",
        "vertical"
      );
