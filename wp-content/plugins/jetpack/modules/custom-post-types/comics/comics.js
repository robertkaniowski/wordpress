  ...
  var Jetpack_Comics = {
    init: function() {
      ...
      if (!("FileReader" in window && "File" in window)) {
        $("#jetpack-comic-drop-zone .dragging").text(
          Jetpack_Comics_Options.labels.unsupported
        );
        $(document)
    ...
    onDrop: function(event) {
      ...
      var files = Jetpack_Comics.filterImageFiles(
      ...
      $.ajax({
        url:
          Jetpack_Comics_Options.writeURL +
          "&nonce=" +
          Jetpack_Comics_Options.nonce,
        data: formData,
        processData: false,
        contentType: false,
        type: "POST",
