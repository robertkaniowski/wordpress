  ...
  function init(args) {
    ...
    function callCallback(callbackName, element, eventData) {
      ...
      $.each(settings[callbackName], function(idx, callback) {
        result.value =
          callback.call(jmpress, element, eventData) || result.value;
      });
      return result.value;
    }
    /**
    ...
    function select(el, type) {
      ...
      $(jmpress).addClass(
        (current.jmpressClass = "step-" + $(delegated).attr("id"))
      );
...
(function($, document, window, undefined) {
      ...
      transform: function(el, data) {
        ...
        $.each(data, function(idx, item) {
          ...
          if (item[0] === "translate") {
            ...
            for (i = 0; i < 3; i++) {
              transform +=
                " scale" + coord[i] + "(" + toCssNumber(item[i + 1] || 1) + ")";
            }
          }
        });
  ...
  $.jmpress("setActive", function(element, eventData) {
    ...
    for (
      var i = eventData.current.maxNestedDepth;
  ...
  $.jmpress("applyTarget", function(active, eventData) {
    var target = eventData.target,
      ...
      zoomin =
        target.perspectiveScale * 1.3 < eventData.current.perspectiveScale,
      zoomout =
        target.perspectiveScale > eventData.current.perspectiveScale * 1.3;
...
(function($, document, window, undefined) {
  ...
  $jmpress("initStep", function(step, eventData) {
    eventData.stepData.exclude =
      eventData.data.exclude &&
      ["false", "no"].indexOf(eventData.data.exclude) === -1;
  });
...
(function($, document, window, undefined) {
  ...
  /* HOOKS */
  $jmpress("initStep", function(step, eventData) {
    eventData.stepData.src =
      $(step).attr("href") || eventData.data.src || false;
      ...
      function(event) {
        ...
        if (event.which === 9) {
          ...
          if (!eventTarget.closest(jmpress.jmpress("active")).length) {
            ...
            nextFocus = eventTarget.near(
              keyboardSettings.tabSelector,
              event.shiftKey
            );
            if (
              !$(nextFocus)
...
(function($, document, window, undefined) {
  ...
  $.jmpress("afterInit", function(nil, eventData) {
    ...
    if (eventData.settings.viewPort.zoomBindMove) {
      $(eventData.settings.fullscreen ? document : this)
        .bind("mousedown" + eventData.current.viewPortNamespace, function(
          event
        ) {
        ...
        .bind("mousemove" + eventData.current.viewPortNamespace, function(
          event
        ) {
  ...
  $.jmpress("setActive", function(step, eventData) {
    ...
    var viewPortMaxScale =
      eventData.stepData.viewPortMaxScale || viewPort.maxScale;
    var viewPortMinScale =
      eventData.stepData.viewPortMinScale || viewPort.minScale;
    // Correct the scale based on the window's size
    var windowScaleY =
      viewPortHeight && $(eventData.container).innerHeight() / viewPortHeight;
    var windowScaleX =
      viewPortWidth && $(eventData.container).innerWidth() / viewPortWidth;
    var windowScale =
    ...
    if (windowScale) {
      ...
      var zoomableSteps =
        eventData.stepData.viewPortZoomable ||
...
(function($, document, window, undefined) {
  ...
  function addUndefined(target, values, prefix) {
    for (var name in values) {
      ...
      if (prefix) {
        targetName =
          prefix + targetName.substr(0, 1).toUpperCase() + targetName.substr(1);
      }
...
(function($, document, window, undefined) {
  ...
  $.jmpress("applyStep", function(step, eventData) {
    ...
    if (current._after === undefined && current._on.length === 0) {
      var startStep =
        find(listOfSubsteps, eventData.stepData.startSubstep) || 0;
      ...
      $.each(substepsData.list, function(idx, activeSubsteps) {
        $.each(activeSubsteps, function(idx, substep) {
          if (substep.substep.info.willClass) {
            $(substep.substep.element).removeClass(
              substep.substep.info.willClass
            );
          }
  ...
  $.jmpress("setActive", function(step, eventData) {
    ...
    if (eventData.substep === undefined) {
      eventData.substep =
        eventData.reason === "prev" ? substepsData.list.length - 1 : 0;
    }
    ...
    if (dur && dur > 0) {
      if (durationSettings.barSelector) {
        var css = {
          transitionProperty: durationSettings.barProperty,
          transitionDuration:
            dur - (settings.transitionDuration * 2) / 3 - 100 + "ms",
      ...
      current.durationTimeout = setTimeout(function() {
        var action =
          eventData.stepData.durationAction || durationSettings.defaultAction;
        $(jmpress).jmpress(action);
      }, dur);
    }
  });
...
(function($, document, window, undefined) {
      ...
      window.addEventListener("message", function(event) {
        ...
        try {
          ...
          switch (json.type) {
            case "select":
              $.each(
                eventData.settings.presentationMode.transferredValues,
                function(idx, name) {
  ...
  $jmpress("register", "presentationPopup", function() {
    function trySend() {
      jmpress.jmpress("current").presentationPopupTimeout = setTimeout(
        trySend,
        100
      );
      try {
