...
(function() {
  ...
  Scroller = function(settings) {
    ...
    this.handle.innerHTML =
      "<span><button>" + text.replace("\\", "") + "</button></span>";
  ...
  Scroller.prototype.render = function(response) {
    var childrenToAppend = Array.prototype.slice.call(
      response.fragment.childNodes
    );
  ...
  Scroller.prototype.thefooter = function() {
    ...
    if (sourceBottom !== targetBottom) {
      self.animate(function(progress) {
        var currentBottom =
          sourceBottom + (targetBottom - sourceBottom) * progress;
    ...
    xhr.onload = function() {
      ...
      if (response.scripts && Array.isArray(response.scripts)) {
        response.scripts.forEach(function(item) {
          ...
          if (
            "wp-mediaelement" === item.handle &&
            "undefined" === typeof mejs
          ) {
            self.wpMediaelement = {};
            ...
            document
              .getElementsByTagName(elementToAppendTo)[0]
              .appendChild(script);
          }
        });
      }
      ...
      if (
        "object" === typeof response.postflair &&
        "object" === typeof WPCOM_sharing_counts
      ) {
        WPCOM_sharing_counts = self.extend(
  ...
  Scroller.prototype.initializeMejs = function(e) {
    ...
    audioVideoElements = document.querySelectorAll(
  ...
  Scroller.prototype.measure = function(element, expandClasses) {
     ...
     */
    var factor =
      (Math.min(maxBottom, viewportMiddle) - Math.max(minTop, 0)) /
      viewportMiddle;
  ...
  Scroller.prototype.ensureFilledViewport = function() {
    ...
    self.body.removeEventListener(
      "is.post-load",
      self.checkViewportOnLoadBound
    );
