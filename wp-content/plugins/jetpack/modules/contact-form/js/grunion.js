...
FB.ContactForm = (function() {
  ...
  function hidePopup() {
    try {
      ...
      if (typeof window.parent.document.body.style.maxHeight === "undefined") {
        //if IE 6
        jQuery("body", "html", window.parent.document).css({
          height: "auto",
          width: "auto"
        });
  ...
  function updateLabel() {
    try {
      ...
      if (thisLabel.length === 0) {
        jQuery("#fb-new-field" + thisId + " label .label-text").text(
          GrunionFB_i18n.newLabel
        );
      } else {
  ...
  function updateOption(that) {
    try {
      ...
      // Update preview
      if (thisType === "radio") {
        jQuery("#fb-radio-" + thisId + "-" + thisOptionid + " span").text(
          thisOptionValue
        );
      } else if ("checkbox-multiple" === thisType) {
        jQuery(
          "#fb-checkbox-multiple-" + thisId + "-" + thisOptionid + " span"
        ).text(thisOptionValue);
      } else {
  ...
  function updateType(thisType, thisLabelText, thisRequired) {
    try {
      ...
      if (!thisLabelText) {
        thisLabelText = jQuery(
          "#fb-new-field" + thisId + " .label-text"
        ).text();
      }
      ...
      var thisCheckboxMultipleRemove =
        '<div class="fb-remove fb-remove-small" id="' + thisId + '"></div>';
      var thisText =
        '<input type="text" id="fb-field' +
        thisId +
        '" "disabled="disabled" />';
      var thisTextarea =
        '<textarea id="fb-field' +
        thisId +
  ...
  return {
    ...
    init: function() {
      ...
      if (
        ...
        jQuery("#wp-content-wrap", window.parent.document).hasClass(
          "tmce-active"
        )
      ) {
        var win = window.dialogArguments || opener || parent || top;
