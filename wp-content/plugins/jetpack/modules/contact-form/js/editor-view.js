  ...
  wp.mce.grunion_wp_view_renderer = {
    ...
    edit: function(data, update_callback) {
      var shortcode_data = wp.shortcode.next(this.shortcode_string, data),
        ...
        $view = $tinyMCE_document
          .find(".wpview.wpview-wrap")
          .filter(function() {
            return $(this).attr("data-mce-selected");
          }),
        $editframe = $(
          '<iframe scrolling="no" class="inline-edit-contact-form" />'
        ),
        index = 0,
        named,
        fields = "",
      ...
      // Render the fields.
      while (
        (field = wp.shortcode.next("contact-field", shortcode.content, index))
      ) {
        index = field.index + field.content.length;
        named = field.shortcode.attrs.named;
        if (named.options && "string" === typeof named.options) {
        ...
        var stylesheet_url =
          ...
          $dashicons_css = $(
            '<link rel="stylesheet" href="' +
              grunionEditorView.dashicons_css_url +
              '" />'
          );
