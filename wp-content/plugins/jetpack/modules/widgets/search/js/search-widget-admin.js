...
(function($, args) {
  var defaultFilterCount =
    "undefined" !== typeof args && args.defaultFilterCount
  ...
  $(document).ready(function() {
    ...
    if ("undefined" !== typeof analytics && args.tracksUserData) {
      analytics.initialize(
        args.tracksUserData.userid,
        args.tracksUserData.username
      );
    }
  });
  ...
  function generateFilterTitlePlaceholder(container) {
    ...
    if ("taxonomy" === type) {
      placeholder = container
        .find(".taxonomy-select option:selected")
        .text()
        .trim();
      ...
      placeholder = container
        .find(".filter-select option:selected")
        .text()
        .trim();
    }
  ...
  var setListeners = function(widget) {
    ...
    widget.on(
      "change",
      ".jetpack-search-filters-widget__sort-order",
      function() {
        var eventArgs = {
          is_customizer: args.tracksEventData.is_customizer
        };
    ...
    widget.on(
      "change",
      ".jetpack-search-filters-widget__taxonomy-select select",
      function() {
        var eventArgs = {
          is_customizer: args.tracksEventData.is_customizer
        };
    ...
    // add filter button
    widget.on("click", ".jetpack-search-filters-widget__add-filter", function(
      e
    ) {
      e.preventDefault();
        ...
        switch (eventArgs.type) {
          ...
          case "date_histogram":
            eventArgs.dateField = filter
              .find(
                ".jetpack-search-filters-widget__date-histogram-select:first select"
              )
              .val();
            eventArgs.dateInterval = filter
              .find(
                ".jetpack-search-filters-widget__date-histogram-select:nth-child( 2 ) select"
              )
              .val();
            break;
        }
    ...
    $(".jetpack-search-filters-widget__filters")
      .sortable({
        ...
        revert: true,
        cancel:
          "input,textarea,button,select,option,.jetpack-search-filters-widget__controls a",
        change: function() {
          if (wp && wp.customize) {
            wp.customize.state("saved").set(false);
          }
  ...
  $(document).on("widget-updated widget-added", function(e, widget) {
    ...
    widget.off(
      "change",
      ".jetpack-search-filters-widget__date-histogram-select:eq(1) select"
    );
    widget.off(
      "click",
      '.jetpack-search-filters-widget__post-types-select input[type="checkbox"]'
