...
(function($) {
  ...
  $document.on("ready", function() {
    var maybeShowNotice = function(e, policyUrl) {
      var $policyUrl = $(policyUrl || this).closest(
        ".eu-cookie-law-widget-policy-url"
      );

      if (
        $policyUrl.find('input[type="radio"][value="default"]').is(":checked")
      ) {
    ...
    $document.on("widget-updated widget-added", function(e, widget) {
      ...
      if (widgetId.indexOf("eu_cookie_law_widget") !== -1) {
        maybeShowNotice(
          null,
          $("#" + widgetId + " .eu-cookie-law-widget-policy-url")
        );
      }
    });
