...
(function(api, wp, $) {
  ...
  $document.ready(function() {
    ...
    $document.on("widget-synced widget-updated", function(
      ...
      var widgetForm = widgetContainer.find(
  ...
  function syncProductLists() {
    var request = wp.ajax.post(
      "customize-jetpack-simple-payments-buttons-get",
      {
  ...
  function clearForm(widgetForm) {
    return function(event) {
      ...
      widgetForm
        .find(
          ".jetpack-simple-payments-add-product, .jetpack-simple-payments-edit-product"
        )
        .attr("disabled", "disabled");
  ...
  function enableFormActions(widgetForm) {
    var isFormVisible = widgetForm
      .find(".jetpack-simple-payments-form")
      .is(":visible");
    var isProductSelectVisible = widgetForm
      .find(".jetpack-simple-payments-products")
      .is(":visible"); //areProductsVisible ?
    var isEdit =
      widgetForm.find(".jetpack-simple-payments-form-action").val() === "edit";
  ...
  function selectImage(widgetForm) {
    return function(event) {
      ...
      mediaFrame.on("select", function() {
        ...
        //hide placeholder
        widgetForm
          .find(".jetpack-simple-payments-image-fieldset .placeholder")
          .hide();
  ...
  function updateProductImage(widgetForm) {
    ...
    var newImageSrc = widgetForm
      .find(".jetpack-simple-payments-form-image-src")
      .val();

    var placeholder = widgetForm.find(
      ".jetpack-simple-payments-image-fieldset .placeholder"
    );
  ...
  function isFormValid(widgetForm) {
    ...
    var postTitle = widgetForm
      .find(".jetpack-simple-payments-form-product-title")
      .val();
    if (!postTitle) {
      widgetForm
        .find(".jetpack-simple-payments-form-product-title")
    ...
    var productPrice = widgetForm
      .find(".jetpack-simple-payments-form-product-price")
      .val();
    ...
    var precision =
      widgetForm
        .find(".jetpack-simple-payments-form-product-currency")
        .val() === "JPY"
        ? 0
        : 2;
    ...
    var productEmail = widgetForm
      .find(".jetpack-simple-payments-form-product-email")
      .val();
  ...
  function saveChanges(widgetForm) {
    return function(event) {
      event.preventDefault();
      var productPostId = widgetForm
        .find(".jetpack-simple-payments-form-product-id")
        .val();
      ...
      var request = wp.ajax.post(
        "customize-jetpack-simple-payments-button-save",
        {
          "customize-jetpack-simple-payments-nonce":
            api.settings.nonce["customize-jetpack-simple-payments"],
          customize_changeset_uuid: api.settings.changeset.uuid,
      ...
      request.done(function(data) {
        ...
        var productOption = select.find(
          'option[value="' + productPostId + '"]'
        );
  ...
  function deleteProduct(widgetForm) {
    return function(event) {
      ...
      var request = wp.ajax.post(
        "customize-jetpack-simple-payments-button-delete",
        {
          "customize-jetpack-simple-payments-nonce":
            api.settings.nonce["customize-jetpack-simple-payments"],
          customize_changeset_uuid: api.settings.changeset.uuid,
      ...
      request.done(function() {
        var productList = widgetForm.find(
          "select.jetpack-simple-payments-products"
        )[0];
