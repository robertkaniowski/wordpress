...
wp.isJetpackWidgetPlaced = function(placement, widgetName) {
  return (
    ...
    0 === placement.partial.widgetId.indexOf(widgetName)
  );
};
    ...
    if (wp && wp.customize && wp.customizerHasPartialWidgetRefresh()) {
        ...
        if (placement.container) {
          ...
          // Refresh Facebook XFBML
            ...
            FB.XFBML.parse(placement.container[0], function() {
              var $fbContainer = $(placement.container[0]).find(
                  ".fb_iframe_widget"
                ),
              ...
              $fbContainer
                .find("span")
                .css({ width: fbWidth, height: fbHeight });
              setTimeout(function() {
          ...
          // Refresh Twitter
            ...
            wp.isJetpackWidgetPlaced(placement, "eu_cookie_law_widget")
          ) {
      ...
      wp.customize.selectiveRefresh.bind("partial-content-moved", function(
        ...
        if (placement.container) {
          ...
          if (
            wp.isJetpackWidgetPlaced(placement, "twitter_timeline") &&
            placement.container.find("iframe.twitter-timeline:not([src]):first")
              .length
          ) {
